...
public class OHLC5MinJob extends QuartzJobBean {
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		// logger.info("running ohlc 5 min job" );
		if (KeyCache.containAccessToken() && !DataCache.getSubscriptions().isEmpty()) {

			insertOhlc5Min();
			insertichimoku5Min();
			// logger.info("ohlc 5 min job update size : " + batchUpdate.length);
	...
	private void populateIchimokuCache(IchimokuData ichimokuData) {
		...
		if (IchimokuCache.contains5Min(symbol)) {
			indicator = IchimokuCache.get5Min(symbol);
			...
			IchimokuCache.add5Min(symbol, indicator);
			ichimokuData.checkForCrossOver(indicator);
			int checkAlerts = indicator.checkAlerts();
	...
	private List<IchimokuData> loadIchimokuData() {
		String sql = "select ichimoku5min.*, ohlc5min.open, ohlc5min.high, ohlc5min.low, ohlc5min.close from ichimoku5min, ohlc5min "
				+ " where ichimoku5min.symbol in (:symbol) AND ohlc5min.symbol = ichimoku5min.symbol AND ohlc5min.time = ichimoku5min.time  "
		...
		String chikouSQL = "select * from ichimoku5min  where symbol  = :symbol  order by time desc limit 1  offset 52";
	...
	private void insertOhlc5Min() {
		String sql = "INSERT INTO ohlc5min (time, symbol, open,close, high, low)"
				+ " SELECT time_bucket('5 minutes', time) AS five_min, ? as symbol,"
				+ " first(ltp, time) AS open, last(ltp, time) AS close, MAX(ltp) AS high, MIN(ltp) AS low"
				+ " FROM data WHERE time > NOW() - interval '5 minutes'   AND symbol = ?"
				+ " GROUP BY five_min  ORDER BY five_min DESC limit 1";

		executeOhlcSql(sql);
	}

	private void insertichimoku5Min() {
		String sql = "INSERT into ichimoku5min (time, symbol, tenkan, kijun, spana, spanb, chikou) "
				...
				+ "	 FROM ohlc5min " + " WHERE  symbol= ? AND time > NOW() - interval '7 days' "
